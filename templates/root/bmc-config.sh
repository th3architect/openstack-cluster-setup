...
# This script sets up the IPMI interface on VICCI nodes
# IPMI shares NIC1 with the node's primary interface.
#
# Each node's IPMI is assigned 192.168.0.<index>
# E.g., node37 is 192.168.0.37

#yum -y install freeipmi ipmitool OpenIPMI
apt-get -y install freeipmi-tools ipmitool openipmi chkconfig
chkconfig ipmi on
service ipmi start

INDEX=$(hostname|perl -ne 'm|node(\d+)\.(.*)\.vicci\.org|&&print $1')

cat <<EOF|sed "s/INDEX/$INDEX/"|bmc-config --commit
#
...
Section Lan_Conf
	...
	## Give valid IP address
	Default_Gateway_IP_Address                    192.168.0.254
	## Give valid MAC address
